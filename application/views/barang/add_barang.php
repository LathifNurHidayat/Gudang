<div class="col">
    <div class="page-pretitle">
        Halaman
    </div>
    <h2 class="page-title">
        Tambah Barang
    </h2>
</div>

<div class="col-8 mt-3">
    <div class="card">
        <div class="card-body">

            <?= $this->session->flashdata('msg'); ?>

            <?= form_open('barang/insert_barang', ['class' => 'm-3']); ?>

            <div class="mb-3">
                <label class="form-label">Jenis Barang</label>
                <select class="form-select" name="id_jenis_barang">
                    <option value="">Pilih jenis barang</option>
                    <?php foreach ($jenis_barang as $jenis): ?>
                        <option value="<?= $jenis->id_jenis_barang ?>" <?= set_select('id_jenis_barang', $jenis->id_jenis_barang) ?>>
                            <?= $jenis->nama_jenis_barang ?>
                        </option>
                    <?php endforeach; ?>
                </select>
                <small class="text-danger"><i><?= form_error('id_jenis_barang') ?></i></small>
            </div>

            <div class="mb-3">
                <label class="form-label">Nama Barang</label>
                <input type="text" class="form-control" name="nama_barang" value="<?= set_value('nama_barang') ?>">
                <small class="text-danger"><i><?= form_error('nama_barang') ?></i></small>
            </div>

            <div class="mb-3">
                <label class="form-label">Harga</label>
                <input type="number" class="form-control" name="harga" value="<?= set_value('harga') ?>">
                <small class="text-danger"><i><?= form_error('harga') ?></i></small>
            </div>

            <div class="mb-3">
                <label class="form-label">Stok</label>
                <input type="number" class="form-control" name="stok" value="<?= set_value('stok') ?>">
                <small class="text-danger"><i><?= form_error('stok') ?></i></small>
            </div>

            <div class="d-flex justify-content-between">
                <a href="<?= base_url('barang/index') ?>" class="btn btn-warning">Kembali</a>
                <button type="submit" class="btn btn-primary">
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24"
                        stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round"
                        stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                        <line x1="10" y1="14" x2="21" y2="3" />
                        <path d="M21 3l-6.5 18a0.55 .55 0 0 1 -1 0l-3.5 -7l-7 -3.5a0.55 .55 0 0 1 0 -1l18 -6.5" />
                    </svg>
                    Kirim
                </button>
            </div>

            <?= form_close(); ?>
        </div>
    </div>
</div>